...
export default function ManagerLayout() {
  ...
  useEffect(() => {
    ...
    // Require an auth session; avoid forcing "signin completed" flag.
  ...
  const approver = session.access === "approver" || session.role === "approver";

  // If an approver somehow hits the viewer layout, push them to the approver portal.
  useEffect(() => {
    if (approver && !location.pathname.startsWith("/manager-approver-dashboard")) {
      navigate("/manager-approver-dashboard", { replace: true });
    }
  }, [approver, location.pathname, navigate]);
